...
def plotLearning(x, scores, epsilons, filename, window):
    ...
    N = len(scores)
    runnning_avg = np.empty(N)
    for i in range(N):
        runnning_avg[t] = np.mean(scores[max(0, t - window):(t + 1)])

    ax2.scatter(x, runnning_avg, color="C1")
...
def plotLearningNoEpsilons(score, filename, x=None, window=5):
    N = len(scores)
